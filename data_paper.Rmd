...
all_lang <- grep("^lang", names(languages_data), value = TRUE)
...
```{r}
replacement = c("access_no_digital"="No digital",
      "access_phone"="Telephone",
      "access_tablet"="Tablet",
      "access_laptop"="Laptop",
      "access_spotlight"="Spotlight",
      "access_drone"="Drone")
kable(access_data %>%
  select(
    starts_with("access_phone"),
    starts_with("access_table"),
    starts_with("access_laptop"),
    starts_with("access_spotlight"),
    starts_with("access_no_digital"),
...
kable(access_data %>%
  ...
  summarise(
    ...
    mutate(Answers = str_replace(access_phone_type, "0", "0: No answer"),
            order = if_else(Answers == "0: No answer", 1, 0)) %>%
    arrange(order, Answers) %>%
...
```{r}
replacement = c("access_no_phone_interdiction" = "Interdiction",
      "access_no_phone_husband" = "Husband autorisation",
      "access_no_phone_cultur" = "Perception",
      "access_no_phone_child" = "Child",
      "access_no_phone_illiterate" = "Illiterate",
      "access_no_phone_satisfaction" = "Satisfaction",
      "access_no_phone_internet" = "Internet",
      "access_no_phone_network" = "Network",
      "access_no_phone_electricity" = "Electricity",
      "access_no_phone_money" = "Money",
      "access_no_phone_usage" = "Can't use",
      "access_nà_phone_friend" = "Friend",
      "access_no_phone_danger" = "Dangerous",
      "access_no_phone_desire" ="Not desire",
      "access_no_phone_time" = "Time consuming",
      "access_no_phone_need" ="Don't need")
kable(access_data %>%
  select(
    starts_with("access_no_phone")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
...
kable(access_data %>%
  ...
  summarise(
    ...
    mutate(Answers = str_replace(access_recharge_service, "0", "0: No answer")) %>%
  select(Answers, Number),format = "latex", booktabs = TRUE, caption = "Do you have access to credit top-up or communication package services in your village or locality?") %>%
  kable_styling(latex_options = c("HOLD_position"),full_width = TRUE)
```
...
## Usage
```{r}
replacement = c("usage__google"="Google",
      "usage__whatsapp"="Whatsapp",
      "usage__facebook"="Facebook",
      "usage__youtube"="Youtube",
      "usage__linkedin"="Linkedin",
      "usage__instagramm"="Instagram",
      "usage__tktok"="Tiktok",
      "usage__snap"="Snapchat",
      "usage__other_no_agri"="Other no agri",
      "usage__other_agri"="Other agri",
      "usage_phone_momo" = "Mobile Money")

kable(usage_data %>%
  select(
    starts_with("usage__google"),
    starts_with("usage__whatsapp"),
    starts_with("usage__facebook"),
    starts_with("usage__youtube"),
    starts_with("usage__linkedin"),
    starts_with("usage__instagramm"),
    starts_with("usage__tktok"),
    starts_with("usage__snap"),
    starts_with("usage__other_no_agri"),
    starts_with("usage__other_agri"),
...
df_long <- usage_data %>%
  select(matches("^(usage_call_sms|usage_vocal|usage_photos|usage_videos|usage_messages)_")) %>%
  pivot_longer(cols = everything(),
               names_to = c("type", "Usage"),
               names_pattern = "^(usage_call_sms|usage_vocal|usage_photos|usage_videos|usage_messages)_(.+)$",,
               values_to = "valeur")
...
```{r}
replacement = c("usage_call_sms"="Call/Sms","usage_vocal"="Vocal", "usage_messages"="Messages","usage_photos"="Photos", "usage_videos"="Videos")
prefixes <- c("usage_call_sms", "usage_vocal", "usage_messages", "usage_photos", "usage_videos")
...
```{r}
replacement = c( "usage_info_topic_agri_tech"="Agri technique",
      "usage_info_topic_crop_protect"="Crop protection",
      "usage_info_topic_price_product"="Price of product",
      "usage_info_topic_climate_data"="Climate data",
      "usage_info_topic_market"="Market access",
      "usage_info_topic_tech"="Technique",
      "usage_info_topic_finance"="Financial management",
      "usage_info_topic_land"="Land law",
      "usage_info_topic_other"="Other")

kable(usage_data %>%
  select(
    starts_with("usage_info")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
...
```{r}
replacement = c("usage_buy_product_seed"="Seed",
      "usage_buy_product_dropping"="Dropping",
      "usage_buy_product_fertilizer"="Fertilizer",
      "usage_buy_product_compost"="Compost",
      "usage_buy_product_crop_protect"="Plant proctection",
      "usage_buy_product_material_agri"="Agri material",
      "usage_buy_product_labour"="Labour",
      "usage_buy_product_other"="Other")
kable(usage_data %>%
  select(
    starts_with("usage_buy")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
...
```{r}
replacement = c("usage_knowledge_agri_quest"="Agri question",
      "usage_knowledge_whatsapp_quest"="Whatsapp question",
      "usage_knowledge_agri_video"="Agri videos",
      "usage_knowledge_search_read"="Read research",
      "usage_knowledge_acces_social_media"="Social media ",
      "usage_knowledge_acces_specific_platform"="Specific platform",
      "usage_knowledge_acces_youtube_search"="Research on youtube",
      "usage_knowledge_acces_internet_search"="Internet research",
      "usage_knowledge_acces_sd_card"="Pre-filled SD cards",
      "usage_knowledge_acces_bluetooth"="Bluetooth file transfer")
kable(usage_data %>%
  select(
    starts_with("usage_knowledge")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
...
```{r}
replacement = c("usage_group_type_veg_producers"="Vegetable producers",
      "usage_group_type_corp_member"="Cooperative members",
      "usage_group_type_assistance"="Knowledge exchange",
      "usage_group_type_advisor_agri"="Agri advisory",
      "usage_group_type_other"="Other")
kable(usage_data %>%
  select(
    starts_with("usage_group")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
...
```{r}
replacement = c("usage_momo"="Momo", "usage_market"="Market", "usage_buy"="Buy", "usage_knowledge"="Knowledge", "usage_info"="Information", "usage_group"="Group")
prefixes <- c("usage_momo", "usage_market","usage_buy", "usage_knowledge", "usage_info", "usage_group")
...
```{r}
replacement = c("usage_recruit_rent_temporal"="Temporal recruitment","usage_recruit_rent_machine_rental"="Rent machine")
kable(usage_data %>%
  select(
    starts_with("usage_recruit_rent")
  ) %>%
  pivot_longer(cols = everything(), names_to = "variable", values_to = "valeur") %>%
  group_by(variable) %>%
  summarise(
